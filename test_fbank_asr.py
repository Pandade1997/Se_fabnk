...
from src.data import load_textset, load_dataset, load_noisy_dataset
...
def main():
    _, tt_set, feat_dim, vocab_size, tokenizer, msg = load_dataset(args.njobs, args.gpu, args.pin_memory,
    ...
    with torch.no_grad():
        ...
        for input in tqdm(tt_set):
            tt_noisy_set, feat_dim, vocab_size, tokenizer = load_noisy_dataset("test", input[0], args.njobs,
                                                                               args.gpu,
                                                                               args.pin_memory,
                                                                               config['hparas']['curriculum'] > 0,
                                                                               **config['data_noisy'])
