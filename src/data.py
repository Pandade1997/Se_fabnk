...
from corpus.librispeech import LibriNoisyDataset
...
def create_noisy_dataset(job, input, tokenizer, ascending, name, path, bucketing, batch_size,
                         train_split=None, dev_split=None, test_split=None):
    ''' Interface for creating all kinds of dataset'''
    ...
    if train_split is not None:
        ...
        tr_set = LibriNoisyDataset(job, input, mode, path, train_split, tokenizer,
                                   bucket_size, ascending=ascending)
        ...
        bucket_size = batch_size if bucketing and (
            not ascending) else 1
        tt_set = LibriNoisyDataset(job, input, mode, path, train_split, tokenizer,
                           bucket_size, ascending=ascending)
...
def load_noisy_dataset(job, input, n_jobs, use_gpu, pin_memory, ascending, corpus, audio, text):
    ...
    tokenizer = load_text_encoder(**text)
    # Dataset (in testing mode, tr_set=dv_set, dv_set=tt_set)
    tr_set, tr_loader_bs, mode = create_noisy_dataset(job, input, tokenizer, ascending, **corpus)
    # Collect function
    collect_tr = partial(collect_audio_batch,
                         audio_transform=audio_transform, mode=mode)
    # Shuffle/drop applied to training set only
