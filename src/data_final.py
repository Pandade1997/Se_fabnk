...
from corpus.librispeech_final import LibriNoisyDataset
...
def collect_audio_batch(batch, audio_transform, mode):
    ...
    audio_len, file, audio_feat = zip(*[(feat_len, f_name, feat)
                                        for feat_len, f_name, feat in
                                        sorted(zip(audio_len, file, audio_feat), reverse=True,
                                               key=lambda x: x[0])])
    # Zero-padding
...
def create_dataset(ascending, name, path, bucketing, batch_size,
                   train_split=None, dev_split=None, test_split=None):
    ''' Interface for creating all kinds of dataset'''

    # Recognize corpus
    if name.lower() == "librispeech":
        from corpus.librispeech_final import LibriDataset as Dataset
    else:
...
def create_noisy_dataset(job, input, ascending, name, path, bucketing, batch_size,
                         train_split=None, dev_split=None, test_split=None):
    ''' Interface for creating all kinds of dataset'''
    ...
    if train_split is not None:
        ...
        tr_loader_bs = 1 if bucketing and (not ascending) else batch_size
        bucket_size = batch_size if bucketing and (
            not ascending) else 1
        tr_set = LibriNoisyDataset(job, input, mode, path, train_split,
                                   bucket_size, ascending=ascending)

        return tr_set, tr_loader_bs, mode
    else:
        # Testing model
        mode = 'test'
        # Do not use bucketing for test set
        ...
        bucket_size = batch_size if bucketing and (
            not ascending) else 1
        tt_set = LibriNoisyDataset(job, input, mode, path, train_split,
                                   bucket_size, ascending=ascending)
        return tt_set, tr_loader_bs, mode
...
def load_noisy_dataset(job, input, n_jobs, use_gpu, pin_memory, ascending, corpus, audio, text):
    ''' Prepare dataloader for training/validation'''
    ...
    # Dataset (in testing mode, tr_set=dv_set, dv_set=tt_set)
    tr_set, tr_loader_bs, mode = create_noisy_dataset(job, input, ascending, **corpus)
    # Collect function
    collect_tr = partial(collect_audio_batch,
                         audio_transform=audio_transform, mode=mode)
    # Shuffle/drop applied to training set only
